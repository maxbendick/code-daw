...
// TODO multiple files, work from { path: string, content: string }[] input
...
test.skip('node require attempt', () => {
  const sourceCode = `
    window.mySuccess = true
    console.log(5);
  `
  const encoded = _urlEncodeJavaScript(sourceCode)

  require(encoded)

  expect((window as any).mySuccess).toBeTruthy()
})
